  ...
  return (
    <div className="w-full bg-soft-bg/40 py-16 border-b border-light-border/60">
      <div className="w-full px-6 sm:px-12 lg:px-16">
        ...
        <div className="flex overflow-x-auto pb-4 gap-4 md:grid md:grid-cols-4 lg:grid-cols-6 md:gap-6 scrollbar-none snap-x snap-mandatory scroll-smooth -mx-6 px-6 sm:-mx-12 sm:px-12 lg:-mx-16 lg:px-16">
          {categories.map((cat) => (
            <div
              key={cat._id}
              onClick={() => navigate(`/products?category=${cat._id}`)}
              className="flex-shrink-0 w-[135px] sm:w-auto group flex flex-col items-center justify-center p-5 bg-white border border-light-border/60 rounded-3xl hover:border-primary/20 shadow-[0_8px_30px_rgb(0,0,0,0.01)] hover:shadow-[0_16px_35px_rgba(15,157,138,0.04)] hover:-translate-y-1 transition-all duration-300 cursor-pointer snap-start"
            >
              <div className="w-12 h-12 rounded-full bg-soft-bg text-dark-navy group-hover:bg-accent-light group-hover:text-primary flex items-center justify-center transition-all duration-300 shadow-sm border border-light-border/10">
                {getCategoryIcon(cat.name)}
              </div>
              <span className="text-xs sm:text-sm font-bold text-dark-navy mt-3 group-hover:text-primary transition-colors line-clamp-1">
                {cat.name}
              </span>
            </div>
          ))}
